...
#columns_to_convert = c("hotel", "children", "meal", "market_segment", "distribution_channel", #"is_repeated_guest", "reserved_room_type", "assigned_room_type", "deposit_type", #"customer_type", "required_car_parking_spaces")

#hotels_dev[columns_to_convert] <- lapply(hotels_dev[columns_to_convert], as.factor)
#hotels_val[columns_to_convert] <- lapply(hotels_val[columns_to_convert], as.factor)

#factor_vars <- sapply(hotels_dev, is.factor)
#for(var in names(factor_vars[factor_vars])) {
#  levels(hotels_dev[[var]]) <- make.names(levels(hotels_dev[[var]]), unique = TRUE)
#}

#factor_vars <- sapply(hotels_val, is.factor)
#for(var in names(factor_vars[factor_vars])) {
#  levels(hotels_val[[var]]) <- make.names(levels(hotels_val[[var]]), unique = TRUE)
#}
...
A better model was built using lasso-penalized logistic regression. `arrival_date` is a character variable that is difficult to incorporate directly into the model, so we extracted the variable `month` from `arrival_date`, which is a factor of 12 levels. We also included interactions between all main effects in our model. 
...
hotels_logit3 = gamlr(hotels_dev_x, hotels_dev_y, family="binomial")

hotels_lasso = glmnet(hotels_dev_x, hotels_dev_y, alpha = 1)

hotels_dev_x_test = model.matrix(children ~ . - 1 - arrival_date, data=hotels_dev_test)
hotels_dev_y_test = hotels_dev_test$children
  
phat_hotels_lasso = predict(hotels_lasso, newx = hotels_dev_x_test)

```

We produced confusion matrices and calculated TPR, FPR and FDR for the three models. Our model outperforms the baseline models by correctly predicting more positives as well as more negatives. 

```{r q3-5}

# fix problem of unrepresented categories in predict
#is.prone <- function(x) is.factor(x)
...
phat_hotels_logit3 = predict(hotels_logit3, hotels_dev_x_test, type='response')
...
hotels_val_x = model.matrix(children ~ (. - 1 - arrival_date)^2, data=hotels_val)
phat_hotels_logit3 = predict(hotels_logit3, hotels_val_x, type='response')
...
For this question, our goal is to predict whether a mushroom is poisonous. 
