...
ggplot(cv_grid_350) + 
  ...
  geom_vline(xintercept=min_err_350, color="skyblue") +
  scale_x_log10() + 
  labs(x = "K",
        y = "RMSE", 
...
# plot predictions vs x for k=16

knn16 = knnreg(price ~ mileage, data=sclass_350_train, k=16)

sclass_350_test = sclass_350_test %>%
  mutate(price_pred = predict(knn16, sclass_350_test))

ggplot(sclass_350_test) + 
  geom_point(mapping = aes(x = mileage, y = price), alpha=0.5) + 
  geom_line(aes(x = mileage, y = price_pred), color='skyblue', size=1.5) +
  labs(title = "Predicted Price versus Mileage (trim = 350)", x = "Mileage", y = "Price") + 
  theme_minimal()  +
  theme(plot.title = element_text(hjust=0.5))
...
We performed similar procedures for cars with trim levels of 65 AMG. The K value with the minimum RMSE also falls in the range of 8 to 20. To balance between bias and variance, K = 14 was chosen as the optimal K value. 
...
ggplot(cv_grid_65) + 
  geom_line(aes(x=k, y=err)) +
  geom_vline(xintercept=min_err_65, color="skyblue") +
  scale_x_log10() + 
  labs(x = "K",
        y = "RMSE", 
...
```{r q3-5}
# plot predictions vs x for k=14

knn14 = knnreg(price ~ mileage, data=sclass_350_train, k=14)

sclass_65_test = sclass_65_test %>%
  mutate(price_pred = predict(knn14, sclass_65_test))

ggplot(sclass_65_test) + 
  geom_point(mapping = aes(x = mileage, y = price), alpha=0.5) + 
  geom_line(aes(x = mileage, y = price_pred), color='skyblue', size=1.5) +
  labs(title = "Predicted Price versus Mileage (trim = 65 AMG)", x = "Mileage", y = "Price") + 
  theme_minimal()  +
  theme(plot.title = element_text(hjust=0.5))
...
In conclusion, the Trim 350 cars, with an optimal K value of 16, demonstrates a preference for a larger K value compared to the Trim 65 AMG cars, which has an optimal K value of 14. This distinction suggests that the Trim 350 cars benefit from a slightly higher level of complexity to accurately capture its underlying patterns. Upon examining the data set, we found that the Trim 350 sample is larger and has significantly smaller variance in the price variable, which is likely the reason it yields a higher optimal K value.
